...
void Admin::apply12(){
    ...
    while(1){
        ...
        string newname;
        char f;
        LibSys::library::field f;
        cout<<"the isbn of the book whose name you wanna change:";
        cin>>_isbn;
        cout<<"the field of the book whose name you change:(I,N,A,P)";
        cin>>f;
        cout<<"change the name for the book(new name):";
        cin>>newname;
        switch(f){
            case 'I':
                lib->changeBookName(*this,_isbn,LibSys::library::ISBN,newname);
                break;
            case 'N':
                lib->changeBookName(*this,_isbn,LibSys::library::NAME,newname);
                break;
            case 'A':
                lib->changeBookName(*this,_isbn,LibSys::library::AUTHOR,newname);
                break;
            case 'P':
                lib->changeBookName(*this,_isbn,LibSys::library::PRESS,newname);
                break;
            default:
                cout<<"wrong field input!";
                flag=0;
                break;
        }
        if(flag){
